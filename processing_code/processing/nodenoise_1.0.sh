...
#SBATCH --output=../logs/hbcd-no-denoise-100-%A_%a.log
#SBATCH --array=1-529
#SBATCH --tmp=250G
...
SIMG="${HOME}"/images/qsiprep-1.0.0.sif
CODE_DIR="${HOME}"/pipeline_paper/penn-run/code
SUBJECT_CSV="${HOME}"/tier2_local/code/hbcd_complete_qc_demographics.csv
...
OUTPUT_DIR="${HOME}/pipeline_paper/penn-run/derivatives/no-denoise-1.0.0/qsiprep"
mkdir -p "${OUTPUT_DIR}"

# fail whenever something is fishy, use -x to get verbose logfiles
set -e -u -x
subject_row=$(head -n $((SLURM_ARRAY_TASK_ID + 1)) "${SUBJECT_CSV}" | tail -n 1)
subid=$(echo "$subject_row" | python -c "import sys, re; pattern = r'sub-[a-zA-Z0-9]+(?=,|$)'; matches = re.findall(pattern, sys.stdin.read()); print(matches[0] if len(matches) > 0 else 'ERROR')")
sesid=$(echo "$subject_row" | python -c "import sys, re; pattern = r'ses-[a-zA-Z0-9]+(?=,|$)'; matches = re.findall(pattern, sys.stdin.read()); print(matches[0] if len(matches) > 0 else 'ERROR')")
...
cp $CODE_DIR/eddy_params1p0.json ./
...
singularity run \
    ...
    --n-cpus ${NSLOTS} \
    --stop-on-first-crash \
    --eddy-config "$PWD"/eddy_params1p0.json \
