...
check_drbuddi_effect <- function(dice_max) {
  ...
  alt_csv <- file.path("no_drbuddi_qc.csv")
  out_dir <- file.path(".")
  ...
  if (nrow(df_plot) > 0) {
    ...
    if (length(panel_order) == 0) {
      ...
      grid_with_labels <- cowplot::ggdraw() +
        ...
        cowplot::draw_label("TOPUP Only", x = 0.52, y = 0.05, vjust = 1, size = 12) +
        cowplot::draw_label("TOPUP+DRBUDDI", x = 0.03, y = 0.52, angle = 90, vjust = 1, size = 12)

      ggsave(file.path(out_dir, paste0("drbuddi_qc_scatter_facets_dice-max", dice_max, ".svg")), grid_with_labels, width = 7, height = 9, units = "in")
    }
  } else {
  ...
  if (nrow(tests_df) > 0) {
    out_csv_tests <- file.path(out_dir, paste0("drbuddi_qc_metric_tests_summary_dice-max", dice_max, ".csv"))
